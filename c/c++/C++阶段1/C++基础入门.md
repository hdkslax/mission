...
int main() {
	...
	float f1 = 3.14f; //不加f会被默认转换成double类型
	double d1 = 3.14;
...
> 注意1：在显示字符型变量时，用单引号将字符括起来，不要用双引号(双引号表示string)
...
> 注意：C++风格字符串，需要加入头文件==#include\<string> ==。vs2019之后不用加了。
...
{
	// 0. use a seed to generate different random number each time.
	srand((unsigned int)time(NULL));
    // 1. generate a random number
	int num = rand() % 100 + 1; // generate a random number in 1 ~ 100. 
	cout << num << endl;

	// 2. player guess a number
	int guess;

	// 3. check the guess
    // 4. true - exit the game, false - larger / smaller
	while(1){
		cin >> guess;
		if (guess > num) {
...
int main(int argc, char const *argv[])
{
	...
	do {
		...
		lsb = num % 10;
		mid = num / 10; mid = mid % 10;
		msb = num / 100;

		if (msb*msb*msb + mid*mid*mid + lsb*lsb*lsb == num){
			cout << num << endl;
		}
...
int main() {
	...
	//2、可以通过数组名获取到数组首地址
	cout << "数组首地址为： " << (int)arr << endl; // 默认16进制，可用(int)强制转换为十进制，若地址值过大，则需转为(long)
...
//声明
int max(int a, int b);
int max(int a, int b); //此处是第二次声明，不会报错
//定义
int max(int a, int b)
{
	return a > b ? a : b;
...
//swap.h文件
#include<iostream>
using namespace std; //此句和上句可放在头文件中，也可放在源文件中
...
**注意：** 0~255 之间的内存编号是系统占用的，因此不可以访问
...
> 总结1：**定义**结构体时的关键字是struct，不可省略

> 总结2：**创建**结构体变量时，关键字struct可以省略
...
//const使用场景
void printStudent(const student *stu) //加const防止函数体中的误操作 //用地址传递是为了节省内存空间，不需要把整个stu都复制过来，一个pointer只占用8 bytes
{
